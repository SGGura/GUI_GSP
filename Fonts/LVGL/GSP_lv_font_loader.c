...
#include "GSP_lvgl.h"
#include "GSP_lv_font_loader.h"
#include "../../Drivers/GSP_Driver.h"
#include <stdlib.h>

// Использовать обертку для работы с кучей
#include "../../GSP_mem_monitor.h"
#ifndef lv_mem_alloc
#define lv_mem_alloc(size) user_malloc(size)
#endif
#ifndef lv_mem_free
#define lv_mem_free(ptr) user_free(ptr)
#endif

/* Используем GSP_MemoryReadCallback из GSP_Driver */
...
/* Структура для работы с внешней памятью через GSP_MemoryReadCallback */
typedef struct
{
	void *ext_data;           /* Указатель на структуру проекта (EXTDATA и т.п.) */
	unsigned long current_offset;
} flash_file_t;

// Типы для совместимости
typedef flash_file_t lv_fs_file_t;
...
// Вспомогательные функции для работы с FLASH
static lv_fs_res_t flash_read(lv_fs_file_t * fp, void * buf, uint32_t btr, uint32_t * br);
static lv_fs_res_t flash_seek(lv_fs_file_t * fp, uint32_t pos, uint8_t mode);
...
/**
 * Loads a `lv_font_t` object from FLASH memory
 * @param ext_data pointer to EXTDATA structure containing font address
 * @return a pointer to the font or NULL in case of error
 */
lv_font_t * lv_font_load(const char * font_name)
{
	/* font_name - указатель на структуру проекта (EXTDATA и т.п.), приведённый к char* */
	void *ext_data = (void *)font_name;

	if (ext_data == NULL)	return NULL;

	lv_fs_file_t file;
	file.ext_data = ext_data;
...
static lv_fs_res_t flash_read(lv_fs_file_t * fp, void * buf, uint32_t btr, uint32_t * br)
{
	if (fp == NULL || fp->ext_data == NULL || buf == NULL)
		return LV_FS_RES_ERR;

	{
		GSP_MemoryReadFn read_fn = GSP_GetMemoryReadCallback();
		if (read_fn)
			read_fn(fp->ext_data, fp->current_offset, (unsigned short)btr, buf);
	}

	if (br != NULL) 	*br = btr; // Устанавливаем количество прочитанных байт

	fp->current_offset += btr; // Увеличиваем смещение на запрошенное количество байт
...
static lv_fs_res_t flash_seek(lv_fs_file_t * fp, uint32_t pos, uint8_t mode)
{
	...
	{
		fp->current_offset = pos;
	}
	// Можно добавить LV_FS_SEEK_CUR и LV_FS_SEEK_END при необходимости
...
static unsigned int read_bits(bit_iterator_t * it, int n_bits, lv_fs_res_t * res)
{
	...
	while (n_bits--)
	{
		...
		{
			it->bit_pos = 7;
			*res = flash_read(it->fp, &(it->byte_value), 1, NULL);
			if (*res != LV_FS_RES_OK)
			{
				return 0;
			}
...
{
	flash_seek(fp, start, LV_FS_SEEK_SET);

	uint32_t length;
	char buf[4];

	if (flash_read(fp, &length, 4, NULL) != LV_FS_RES_OK
		|| flash_read(fp, buf, 4, NULL) != LV_FS_RES_OK
...
{
	if (flash_read(fp, cmap_table, font_dsc->cmap_num * sizeof (cmap_table_bin_t), NULL) != LV_FS_RES_OK)
	{
		return false;
	}
	...
	{
		lv_fs_res_t res = flash_seek(fp, cmaps_start + cmap_table[i].data_offset, LV_FS_SEEK_SET);
		if (res != LV_FS_RES_OK)
		{
			return false;
		}
		...
		case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL:
		{
			...
			if (flash_read(fp, glyph_id_ofs_list, ids_size, NULL) != LV_FS_RES_OK)
			{
				return false;
			}
		...
		case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY:
		{
			...
			if (flash_read(fp, unicode_list, list_size, NULL) != LV_FS_RES_OK)
			{
				return false;
			}
			...
			if (cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL)
			{
				...
				if (flash_read(fp, buf, sizeof (uint16_t) * cmap->list_length, NULL) != LV_FS_RES_OK)
				{
					return false;
				}
			}
...
static int32_t load_cmaps(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint32_t cmaps_start)
{
	...
	if (flash_read(fp, &cmaps_subtables_count, sizeof (uint32_t), NULL) != LV_FS_RES_OK)
	{
		return -1;
	}
						  ...
						  uint32_t start, uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
	...
	for (unsigned int i = 0; i < loca_count; ++i)
	{
		...
		lv_fs_res_t res = flash_seek(fp, start + glyph_offset[i], LV_FS_SEEK_SET);
		if (res != LV_FS_RES_OK)
		{
			return -1;
		}
	...
	for (unsigned int i = 1; i < loca_count; ++i)
	{
		...
		if (nbits % 8 == 0)
		{ /*Fast path*/
			if (flash_read(fp, &glyph_bmp[cur_bmp_size], bmp_size, NULL) != LV_FS_RES_OK)
			{
				return -1;
			}
		}
...
static bool lvgl_load_font(lv_fs_file_t * fp, lv_font_t * font)
{
	...
	if (flash_read(fp, &font_header, sizeof (font_header_bin_t), NULL) != LV_FS_RES_OK)
	{
		return false;
	}
	...
	uint32_t loca_count;
	if (flash_read(fp, &loca_count, sizeof (uint32_t), NULL) != LV_FS_RES_OK)
	{
		return false;
	}
	...
	if (font_header.index_to_loc_format == 0)
	{
		...
		{
			uint16_t offset;
			if (flash_read(fp, &offset, sizeof (uint16_t), NULL) != LV_FS_RES_OK)
			{
				failed = true;
				break;
			}
	...
	{
		if (flash_read(fp, glyph_offset, loca_count * sizeof (uint32_t), NULL) != LV_FS_RES_OK)
		{
			failed = true;
		}
	}
...
int32_t load_kern(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint8_t format, uint32_t start)
{
	...
	int32_t padding;
	if (flash_read(fp, &kern_format_type, sizeof (uint8_t), NULL) != LV_FS_RES_OK ||
		flash_read(fp, &padding, 3 * sizeof (uint8_t), NULL) != LV_FS_RES_OK)
	{
		return -1;
	}
	...
	{ /*sorted pairs*/
		...
		if (flash_read(fp, &glyph_entries, sizeof (uint32_t), NULL) != LV_FS_RES_OK)
		{
			return -1;
		}
		...
		if (flash_read(fp, glyph_ids, ids_size, NULL) != LV_FS_RES_OK)
		{
			return -1;
		}

		if (flash_read(fp, values, glyph_entries, NULL) != LV_FS_RES_OK)
		{
			return -1;
		}
	}
	...
	{ /*array M*N of classes*/
		...
		if (flash_read(fp, &kern_class_mapping_length, sizeof (uint16_t), NULL) != LV_FS_RES_OK ||
			flash_read(fp, &kern_table_rows, sizeof (uint8_t), NULL) != LV_FS_RES_OK ||
			flash_read(fp, &kern_table_cols, sizeof (uint8_t), NULL) != LV_FS_RES_OK)
		{
			return -1;
		}
		...
		if (flash_read(fp, kern_left, kern_class_mapping_length, NULL) != LV_FS_RES_OK ||
			flash_read(fp, kern_right, kern_class_mapping_length, NULL) != LV_FS_RES_OK ||
			flash_read(fp, kern_values, kern_values_length, NULL) != LV_FS_RES_OK)
		{
			return -1;
		}
	}
