...
В виджетах (Label, List, Table, Message, Keyboard и т.д.) передаётся указатель на `lv_font_t` (например, `&lv_font_unscii_8` или `&GSP_Arial_16` из `GUI_GSP.h`).
...
- **[LVGL Font Converter](https://lvgl.io/tools/fontconverter)** ([LVGL](https://lvgl.io/)): загружаете TTF/WOFF, задаёте размер, диапазон символов (в т.ч. кириллицу), битность, и получаете:
  - **C array** — файл `.c` с массивом данных; подключаете к проекту и объявляете шрифт в **Fonts.h**.
  - **Binary** — бинарный файл для размещения во внешней памяти и загрузки через **lv_font_load** (формат описан в разделе 10).
...
**Итог по [LVGL](https://lvgl.io/):** библиотека использует те же шрифты и формат, что и LVGL 8.x. Можно применять только встроенные шрифты, только загружаемые из FLASH, или комбинацию; шрифт по умолчанию задаётся **DEFAULT_FONT** в конфиге.
