...
У бібліотеці можна використовувати **два джерела шрифтів**:

1. **Шрифти у форматі C** — сконвертовані в `.c` (як **lv_font_unscii_8.c**): підключаєте файл до проекту, оголошуєте в **Fonts.h**, у коді передаєте `&lv_font_unscii_8`. У мовному пакеті в JSON можна вказати ім'я `"lv_font_unscii_8"` — **файл .bin поруч із JSON не потрібен**, бібліотека підставить вбудований шрифт.
2. **Шрифти з FLASH** — сконвертовані в **Binary** (`.bin`), покладені поруч із `.json` та завантажувані через **lv_font_load** з пакета (див. розділ 3.4 та MEDIA_AND_LANG_PACK).
...
У віджетах (Label, List, Table, Message, Keyboard тощо) передається вказівник на `lv_font_t` (наприклад, `&lv_font_unscii_8` або шрифт із FLASH).
...
- **[LVGL Font Converter](https://lvgl.io/tools/fontconverter)** ([LVGL](https://lvgl.io/)): завантажуєте TTF/WOFF, задаєте розмір, діапазон символів (зокрема кирилицю), бітність і отримуєте:
  - **C array** — файл `.c` з масивом даних (як **lv_font_unscii_8.c** у репозиторії): підключаєте до проекту, оголошуєте в **Fonts.h**; у коді використовуєте `&lv_font_unscii_8`; у JSON мовного пакета можна вказати ім'я `"lv_font_unscii_8"` — **.bin не потрібен**. Додаткові C-шрифти можна додати до таблиці вбудованих у **GSP_Langs.c** (масив `builtin_fonts`).
  - **Binary** — бінарний файл (`.bin`) для розміщення у зовнішній пам'яті та завантаження через **lv_font_load**; кладеться поруч із `.json` при збірці пакета (див. MEDIA_AND_LANG_PACK).
...
**Підсумок по [LVGL](https://lvgl.io/):** бібліотека використовує ті самі шрифти та формат, що й LVGL 8.x. Можна застосовувати **шрифти в C** (`.c` у проекті, як **lv_font_unscii_8.c**), **шрифти з FLASH** (`.bin` у пакеті) або комбінацію; шрифт за замовчуванням задається **DEFAULT_FONT** у конфігу.
